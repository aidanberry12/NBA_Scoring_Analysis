---
title: 'Project Part 3: Data Analysis'
author: "Aidan Berry"
...
allStar <- read.csv('nba_all_star_games.csv')
```
...
```{r visualizations, eval = FALSE, echo = FALSE}
...
```{r numericalVisualizations, eval = FALSE, echo = FALSE}
...
regressor_SVR_Linear = svm(formula = Points ~ .,data = scaled_training_set,type = 'eps-regression', kernel = 'linear')
...
```{r performanceCheck, echo = FALSE}
  resultsTbl <- data.frame(Model = c("Multiple Linear Regression", "KNN Regression", "Random Forest", "Non-Linear SVR", "Linear SVR" ), RMSE = c(rmse_MLR, rmse_KNN, rmse_RF, rmse_NLSVR, rmse_LSVR))
  resultsTbl
